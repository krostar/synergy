...
unit.lib.just.mkRecipe "linters" "lint-nix" {
  ...
  in {
    ...
    groups = ["nix"];
    attributes = ["positional-arguments"];
    parameters = [''+FILES="${builtins.concatStringsSep " " (builtins.map lib.strings.escapeShellArg cfg.settings.include)}"''];
    recipe = ''
      ${lib.meta.getExe' pkgs.nixfmt-rfc-style "nixfmt"} --check --verify --strict -- "$@"
    '';
  };
