...
# MIT License; Copyright (c) 2022 Joshua Gilman. See ../LICENSE.
{
  ...
  */
  ...
  # Render flags GNU-style (`--key value`); replaces the deprecated
  # lib.cli.toGNUCommandLine, and must not shell-quote values because
  # `outfile = "$out"` relies on shell expansion
  mkFlag = key: value:
    if value == true
    then ["--${key}"]
    ...
    then builtins.concatMap (mkFlag key) value
    else ["--${key}" (builtins.toString value)];

  # Build the full CUE command
  flagStr = builtins.concatStringsSep " " (lib.lists.flatten (lib.attrsets.mapAttrsToList mkFlag allFlags));
